...
print "x264farm controller version %s\n" version;;
...
(* Used for printing stuff. Set to the proper value... eventually. *)
...
*)
	let parse_bitrate x = (
		if debug_file then print "Parsing bitrate %s\n" x;
	...
	let parse_zones str =
		if debug_file then print "parse_zones %S\n" str;
		let parse_zone str = (
			if debug_file then print "parse_zone %S\n" str;
			try (
...
(*				let equals = String.index_from str (succ comma2) '=' in*)
				...
				let zone_setup = match String.sub str (succ comma2) 2 with
					...
					| "q=" | "Q=" -> Zone_q number
					| x -> (if debug_file then print "ERROR %S\n" x; Zone_bitrate 1.0)
				in
				if debug_file then (
					match zone_setup with
					| Zone_bitrate x -> print " {%d,%d,b=%f}\n" zone_start zone_end number
					| Zone_q       x -> print " {%d,%d,q=%f}\n" zone_start zone_end number
				);
		...
		let rec parse_zones_rec str =
			if debug_file then print "parse_zones_rec %S\n" str;
...
*)
	...
	let opt_keeptemp = StdOpt.store_true                           () in
	let opt_nocomp   = StdOpt.store_true                           () in
	let opt_logfile  = StdOpt.str_option   ~default:"out-dump.txt" ~metavar:"" () in
	let opt_noadhoc  = StdOpt.store_true                           () in

	let opt_batch     = StdOpt.int_option   ~default:5000         ~metavar:"" () in
	let opt_batchmult = StdOpt.float_option ~default:0.5          ~metavar:"" () in
	let opt_split     = StdOpt.int_option   ~default:250          ~metavar:"" () in
	let opt_thresh    = StdOpt.float_option ~default:20.0         ~metavar:"" () in
	let opt_rethresh  = StdOpt.float_option                       ~metavar:"" () in
	...
	let opt_sizeprec = StdOpt.float_option ~default:1.0          ~metavar:"" () in
...
(*	OptParser.add p ~group:group_general ~short_name:'h' ~long_name:"help"     ~help:"HEYELP!" opt_help;*)
	...
	OptParser.add p ~group:group_control                 ~long_name:"keeptemp" ~help:"Keeps the temporary files from the second pass. NOTE: this will take up a lot of space, and will turn off the --savedisk option" opt_keeptemp;
	...
	OptParser.add p ~group:group_control                 ~long_name:"logfile"  ~help:(Printf.sprintf "Location of the log file (%S)" (Opt.get opt_logfile)) opt_logfile;
	OptParser.add p ~group:group_control                 ~long_name:"noadhoc"  ~help:"Turns off agent discovery" opt_noadhoc;

	let group_1st = OptParser.add_group p "First pass split parameters" in
	OptParser.add p ~group:group_1st                     ~long_name:"batch"     ~help:(Printf.sprintf "How many frames to give out at a time at the beginning (%d)" (Opt.get opt_batch)) opt_batch;
	OptParser.add p ~group:group_1st                     ~long_name:"batchmult" ~help:(Printf.sprintf "Multiplier for --batch for how many frames to give out at the end (%F)" (Opt.get opt_batchmult)) opt_batchmult;
	...
	OptParser.add p ~group:group_1st                     ~long_name:"rethresh"  ~help:(Printf.sprintf "Max threshold for re-splitting (--thresh/2+1/2)") opt_rethresh;
	...
	OptParser.add p ~group:group_2nd                     ~long_name:"sizeprec" ~help:(Printf.sprintf "How precise the file size is to be followed. Values should be between 0 (consistent bitrate) and 1 (precise file size). (%.2f)" (Opt.get opt_sizeprec)) opt_sizeprec;
	...
	let apply_arguments_from_file input_option_file = if Sys.file_exists input_option_file then (
		if debug_file then print "Read stuff from %S\n" input_option_file;
		try (
			let option_handle = open_in input_option_file in
			let rec add_line () = (
				try (
					match input_line option_handle with
					| "" -> (if debug_file then print " (blank line)\n"; add_line ())
					| x when x.[0] = '#' -> (if debug_file then print " Ignored %S\n" x; add_line ())
					| x -> x :: add_line ()
				) with
				| End_of_file -> []
			) in
			let options = Array.of_list ((Sys.argv.(0)) :: (add_line ())) in
			if debug_file then Array.iter (fun x -> print "  %S\n" x) options;
...
*)
	let input_avs = Opt.get opt_avs in
	let input_first_avs = if is_file (Opt.get opt_firstavs) then Opt.get opt_firstavs else if Opt.get opt_firstavs = "" then input_avs else (print "Oops! First AVS file \"%s\" does not exist! Using regular AVS file instead\n" (Opt.get opt_firstavs); input_avs) in
	let input_fast_avs = if is_file (Opt.get opt_fastavs) then Opt.get opt_fastavs else if Opt.get opt_fastavs = "" then input_first_avs else (print "Oops! Fast AVS file \"%s\" does not exist! Using first AVS file instead\n" (Opt.get opt_firstavs); input_first_avs) in
(*
	let input_avs = Opt.get opt_avs in
	let input_first_avs = if Opt.get opt_firstavs = "" then input_avs else Opt.get opt_firstavs in
	let input_fast_avs = if Opt.get opt_fastavs = "" then input_first_avs else Opt.get opt_fastavs in
*)
	...
	(* Let's define the out-dump.txt file here *)
	(* Note that nothing was written to any log file until now *)
	(try
		Controllerinclude.print_handle_ref := Some (open_out (Opt.get opt_logfile));
		print ~screen:false "x264farm controller version %s\n" version;
	with
	...
	let avs2yuv_executable_ref = ref None in

	let ad_hoc_controller_ref = ref None in (* The port that the controller listens on *)
	let ad_hoc_agent_ref = ref None in (* Same for the agents *)
...
*)
	let parse_agent = function
		| Xml.Element ("agent", [("name", name)], agent_guts) -> (
			let ip_ref = ref "127.0.0.1" in
			let port_ref = ref 40700 in
			let number_ref = ref 1 in
			let add_two_ref = ref 0 in
			let parse_agent_attribute = function
				| Xml.Element ("ip",    _, [Xml.PCData x]) -> ip_ref := x
				| Xml.Element ("port",  [("from",x); _], []) -> (port_ref := int_of_string x)
				| Xml.Element ("port",  _, [Xml.PCData x]) -> (port_ref := int_of_string x)
				| Xml.Element ("number",[("pad",y)], [Xml.PCData x]) -> (number_ref := int_of_string x; add_two_ref := int_of_string y)
				| Xml.Element ("number",_, [Xml.PCData x]) -> number_ref := int_of_string x
				| _ -> ()
			in
			List.iter parse_agent_attribute agent_guts;
			{ac_name = name; ac_ip = !ip_ref; ac_port = !port_ref; ac_agents = (!number_ref, !number_ref + !add_two_ref)}
		)
		| _ -> {ac_name = ""; ac_ip = "0.0.0.0"; ac_port = 0; ac_agents = (0,0)}
	in

	let parse_config = function
		| Xml.Element ("temp", _, [Xml.PCData x]) -> temp_dir_ref := x
		| Xml.Element ("agents", _, y) -> agent_list_ref := List.map parse_agent y
		| Xml.Element ("avs2yuv", _, [Xml.PCData x]) -> avs2yuv_executable_ref := Some x
...
*)
		print " agent list:\n";
		List.iter (fun x -> print "  %s = %s:%d (%d,%d)\n" x.ac_name x.ac_ip x.ac_port (fst x.ac_agents) (snd x.ac_agents)) !agent_list_ref
	);
(*
	print "toast\n";
	exit 4;
*)
	{
		...
		o_final_batch_mult = Opt.get opt_batchmult;
		o_split_frame_length = Opt.get opt_split;
		o_split_thresh = Opt.get opt_thresh;
		o_rethresh = (if Opt.is_set opt_rethresh then Opt.get opt_rethresh else Opt.get opt_thresh /. 2. +. 0.5);
		...
		o_size_precision = Opt.get opt_sizeprec;
...
*)
		...
		o_argv = (Array.sub Sys.argv 1 (Array.length Sys.argv - 1));

		o_avs2yuv = !avs2yuv_executable_ref;

		o_adhoc_enabled    = not (Opt.get opt_noadhoc);
		o_adhoc_controller = (match !ad_hoc_controller_ref with | Some x -> x | None -> 0);
		o_adhoc_agent      = (match !ad_hoc_agent_ref      with | Some x -> x | None -> 0);
	}
) in
...
let i = (
	...
	let avs2yuv = (
		...
		| Some x -> x (*override the default handling and assume the user isn't being stupid *)
	) in
...
(*	let open_me = Printf.sprintf "\"%s -frames 1 -raw -o %s \"%s\" 2>&1\""
 *	avs2yuv dev_null o.o_input_avs in*)
	(* This relies on the program/user to put in quotation marks where they belong in the executable *)
...
(*	let file_info_handle = Unix.open_process_in (Printf.sprintf "\"%s -frames 1 -raw -o %s \"%s\" 2>&1\"" avs2yuv dev_null o.o_input_first_avs) in (* dev_null is defined in common/types.ml *)*)
	...
	let fast_every_nth_pixel = int_of_float (float_of_int fast_res_x *. 1.6180340 +. 0.5) in (* Spiralicious! *)
	...
	(* Write the args to some file somewhere *)
	(try
		let time_string = time_string () in
		let argv_string = (
			let quotify x = if String.contains x ' ' then "\"" ^ x ^ "\"" else x in
			Array.fold_left (fun so_far x -> if so_far = "" then quotify x else so_far ^ " " ^ quotify x) "" o.o_argv
		) in
		if is_file (Filename.concat o.o_temp_dir "option_log.txt") then (
			Unix.rename (Filename.concat o.o_temp_dir "option_log.txt") (Filename.concat o.o_temp_dir "option_log.txt.temp")
		) else (
			(* Create the temp file if the original file could not be renamed *)
			let a = open_out (Filename.concat o.o_temp_dir "option_log.txt.temp") in
			close_out a;
		);
		let in_file = open_in (Filename.concat o.o_temp_dir "option_log.txt.temp") in
		let file = open_out (Filename.concat o.o_temp_dir "option_log.txt") in
		output_string file (Printf.sprintf "%s  \"%s\"  %s\n" time_string (Sys.getcwd ()) argv_string);
		let temp_string = String.create 4096 in
		let rec write () = (
			let bytes = input in_file temp_string 0 4096 in
			if bytes <> 0 then (
				output file temp_string 0 bytes;
				write ()
			)
		) in
		write ();
		close_in in_file;
		close_out file;
		Unix.unlink (Filename.concat o.o_temp_dir "option_log.txt.temp");
	with
		x -> (print "Oops. Writing the args failed with: %s" (Printexc.to_string x))
	);
...
(*			print "using compression type 1 (Paeth / Huffman)\n";*)
			...
			let temp_y = String.create (i.i_bytes_y + 8) in
			String.blit "FRAM" 0 temp_y 0 4;
			...
			let temp_u = String.create (bytes_uv + 8) in
			String.blit "FRAM" 0 temp_u 0 4;
			String.blit (Pack.packN bytes_uv) 0 temp_u 4 4;
			(*temp_u.[8] <- '\x00';*)

			(* Compressed Y string *)
			let y_string = String.create (i.i_bytes_y + 8) in
			...
			let u_string = String.create (bytes_uv + 8) in
...
(*				a + b - c*)
			...
			fun num_frames get sock -> (
				for frame = 1 to num_frames do
					really_input get temp_y 8 i.i_bytes_y;
					really_input get temp_u 8 bytes_uv;

					Huff.set_huff_buff_pos y_buff 9;
					Huff.set_huff_buff_pos u_buff 9;

					let (write_me, write_bytes) = if true then (
						if true then (
							(* Do Paeth first, then do Huffman coding *)
							(try

								(* Start from the bottom-right corner and do reverse-scanlines to the top left *)
(*
								for y = i.i_res_y - 1 downto 0 do
									for x = i.i_res_x - 1 downto 0 do
										let a = (if x = 0 then 0 else Char.code temp_y.[8 + y * i.i_res_x + (x - 1)]) in
										let b = (if y = 0 then 0 else Char.code temp_y.[8 + (y - 1) * i.i_res_x + x]) in
										let c = (if x = 0 || y = 0 then 0 else Char.code temp_y.[8 + (y - 1) * i.i_res_x + (x - 1)]) in
										let p = Char.code temp_y.[8 + y * i.i_res_x + x] in
										let encodeme = (p - paeth a b c) land 0xFF in
										temp_y.[8 + y * i.i_res_x + x] <- Char.chr encodeme
									done;
								done;
*)
								Optimizations.do_paeth temp_y i.i_res_x 8;

								(* Now Huffmanize everything *)
								let val_bits_ref = ref (0,0) in
								for q = 8 to 8 + i.i_bytes_y - 1 do
									val_bits_ref := Huff.add_char Huff.y_data y_buff !val_bits_ref (Char.code temp_y.[q]);
								done;
								Huff.write_char y_buff (fst !val_bits_ref);
								Huff.innards_of_huff_buff y_buff

							with
								Huff.Buffer_full -> (temp_y, (i.i_bytes_y + 8))
							)
						) else (
							(* Optimized OCaml-only solution *)
							(try
								let val_bits_ref = ref (0,0) in

								(* Do the upper-left corner *)
								let p = Char.code temp_y.[0 + 8] in
								val_bits_ref := Huff.add_char Huff.y_data y_buff !val_bits_ref p;

								(* Now the top row *)
								let prev_ref = ref p in
								for x = 1 to i.i_res_x - 1 do
									let gnu = Char.code temp_y.[x + 8] in
									let encode_me = (gnu - !prev_ref) land 0xFF in
									val_bits_ref := Huff.add_char Huff.y_data y_buff !val_bits_ref encode_me;
									prev_ref := gnu
								done;

								(* Now the other rows *)
								for y = 1 to i.i_res_y - 1 do
									let y_offset = y * i.i_res_x + 8 in
									let y_offset_prev = (y - 1) * i.i_res_x + 8 in

									(* First column of the row *)
									let p = Char.code temp_y.[y_offset] in
									let b = Char.code temp_y.[y_offset_prev] in
									let encode_me = (p - b) land 0xFF in
									val_bits_ref := Huff.add_char Huff.y_data y_buff !val_bits_ref encode_me;
									...
									let c_ref = ref b in
									for x = 1 to i.i_res_x - 1 do
										let p = Char.code temp_y.[y_offset + x] in
										let b = Char.code temp_y.[y_offset_prev + x] in
										let encode_me = (p - paeth !a_ref b !c_ref) land 0xFF in
										val_bits_ref := Huff.add_char Huff.y_data y_buff !val_bits_ref encode_me;
										a_ref := p;
										c_ref := b;
									done;
...
(* TEMP MD5 *)
(*let md5 = Digest.string temp_y in*)
...
(*					Net.send_full sock ~len:(write_bytes - 8 + 16) (String.sub write_me 0 write_bytes ^ md5);*)
					Net.send_full sock ~len:(write_bytes - 8) write_me;
...
compressed_01_ref := Int64.add !compressed_01_ref (Int64.of_int (write_bytes - 8));
					...
					let (write_me, write_bytes) = if true then (
						(try
							...
							let p = Char.code temp_u.[0 + 8] in
							val_bits_ref := Huff.add_char Huff.u_data u_buff !val_bits_ref (p lxor 128);
							
							(* Top row *)
							let prev_ref = ref p in
							for x = 1 to u_res_x - 1 do
								let gnu = Char.code temp_u.[x + 8] in
								let encode_me = (gnu - !prev_ref) land 0xFF in
								val_bits_ref := Huff.add_char Huff.u_data u_buff !val_bits_ref encode_me;
								prev_ref := gnu
							done;
							
							(* Other rows *)
							for y = 1 to i.i_res_y - 1 do
								let y_offset = y * u_res_x + 8 in
								let y_offset_prev = (y - 1) * u_res_x + 8 in
								
								(* First column *)
								let p = Char.code temp_u.[y_offset] in
								let b = Char.code temp_u.[y_offset_prev] in
								let encode_me = (p - b) land 0xFF in
								val_bits_ref := Huff.add_char Huff.u_data u_buff !val_bits_ref encode_me;
								...
								let c_ref = ref b in
								for x = 1 to u_res_x - 1 do
									let p = Char.code temp_u.[y_offset + x] in
									let b = Char.code temp_u.[y_offset_prev + x] in
									let encode_me = (p - paeth !a_ref b !c_ref) land 0xFF in
									val_bits_ref := Huff.add_char Huff.u_data u_buff !val_bits_ref encode_me;
									a_ref := p;
									c_ref := b;
							...
							Huff.write_char u_buff (fst !val_bits_ref);
							
							Huff.innards_of_huff_buff u_buff
						with
							Huff.Buffer_full -> (temp_u, (bytes_uv + 8))
						)
					) else (
						let rec do_frame_u x y (out_val,out_bits) = (
							if y >= i.i_res_y then (
								...
								let a = (if x = 0 then 128 else Char.code temp_u.[8 + y * u_res_x + (x - 1)]) in
								let b = (if y = 0 then 128 else Char.code temp_u.[8 + (y - 1) * u_res_x + x]) in
								let c = (if x = 0 || y = 0 then 128 else Char.code temp_u.[8 + (y - 1) * u_res_x + (x - 1)]) in
								let p = Char.code temp_u.[8 + y * u_res_x + x] in
								let encode_me = (p - paeth a b c) land 0xFF in
...
(* TEMP MD5 *)
(*let md5 = Digest.string temp_u in*)
(*let md5 = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" in*)
(*print "U MD5 = %s\n" (to_hex md5);*)
(* ! TEMP MD5 *)
...
if false then (
	if !last_printed_ref +. 10.0 < Sys.time () then (
		...
		| x -> failwith "Unknown compression type"
	) in
...
(**************************)
	...
	print ~screen:false " AVS temp dir is %s\n" avs_temp_dir;
	...
	let first_string = Printf.sprintf "%s\n%s\n%d+%d\n%d\n%s = %s\n%s\n" o.o_first (Marshal.to_string o.o_zones []) o.o_seek o.o_frames o.o_preseek o.o_input_first_avs_md5 o.o_input_first_avs_full (Marshal.to_string o.o_second_pass_bitrate []) in
	let first_md5 = to_hex (Digest.string first_string) in
	print ~screen:false " First string is %S, with MD5 %S\n" first_string first_md5;
	...
	print ~screen:false " Second string is %S, with MD5 %S\n" second_string second_md5;
	...
	let do_anyway = (
		if o.o_restart || o.o_force then (
			true
		) else if is_file done_file then (
			let file_option = (try (* Holds the file name, if any *)
				let handle = open_in done_file in
				let line = input_line handle in
				if is_file line then (
					Some line (*Printf.sprintf "This job was already encoded to file \"%s\"\n"*)
				) else (
					None
				)
			with
			...
			| None -> true
			| Some x -> (
				Printf.eprintf "\nThis job was already encoded to file \"%s\"\nDo you want to re-encode? (y/n)\n" x;
				flush stderr;
				let answer = read_line () in
				String.length answer > 0 && Char.lowercase answer.[0] = 'y'
			)
		) else (
			true
		)
	...
	if do_anyway then (
		...
		*)
		...
		if not (is_something first_dir) && is_file first_stats then (
			(* Done! *)
			print ~screen:false "not (is_something first_dir) && is_file first_stats [done]\n";
		) else (
			print ~screen:false "not (not (is_something first_dir) && is_file first_stats) [first pass after making dir]\n";
			if not (is_dir first_dir) then (
				(* Make the dir! *)
				print ~screen:false "not (is_dir first_dir)\n";
...
*)
		...
		if not (is_dir second_dir) then (
			(* Make the dir! *)
			print ~screen:false "not (is_dir second_dir) [second pass after making dir]\n";
...
*)
		...
		if not o.o_keeptemp then (
			print ~screen:false "Removing the second pass temp files\n";
			let delete_later_ref = ref [] in
			let dir_guts = Sys.readdir second_dir in
			Array.iter (fun x ->
				(try
					Unix.unlink (Filename.concat second_dir x);
				with
					_ -> (
						...
						| p -> (
							(match p with
								| Unix.Unix_error (q,y,z) -> print "WARNING: Failed to remove file \"%s\" (%d=%S,%s,%s)\n" (Filename.concat second_dir x) (Obj.magic q) (Unix.error_message q) y z
								| _ -> print "WARNING: Failed to remove file \"%s\"\n" (Filename.concat second_dir x)
							);
							delete_later_ref := x :: !delete_later_ref
						)
					)
				)
			) dir_guts;
			
			if !delete_later_ref <> [] then (
				print "WARNING: Not everything was deleted; waiting 10 seconds and attempting deletion again\n";
				Unix.sleep 10;
				List.iter (fun x ->
					try
						(* Try it twice, just in case *)
					...
					| p -> (
						(match p with
							| Unix.Unix_error (q,y,z) -> print "WARNING: Failed to remove file \"%s\" again (%d=%S,%s,%s)\n" (Filename.concat second_dir x) (Obj.magic q) (Unix.error_message q) y z
							| _ -> print "WARNING: Failed to remove file \"%s\" again\n" (Filename.concat second_dir x)
						);
					)
				) !delete_later_ref
			);
