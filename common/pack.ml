...
(* pack *)
let packc inNum =
	let returnThis = String.create 1 in
	if inNum < 0 then (
...
(*		Printf.printf " %02X %02X %02X %02X\n" orig cleared set gnu;*)
		...
		if gnu > 255 || gnu < 0 then (
			failwith (Printf.sprintf "packBits can't work with %s,%d,%d,%d" (to_hex str) offset num store)
		) else (
